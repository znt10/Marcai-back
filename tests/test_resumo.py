...
def test_a_serie_agrupa_por_dia_LOCAL_mesmo_com_o_settings_em_utc(cenario):
    """O bug que `TruncDate(..., tzinfo=FUSO)` existe para nao ter.

    Sao Paulo esta 3h atras de UTC. Um corte as 21h de sabado e' `2026-09-06
    00:00Z` — agrupado pelo dia UTC, ele aparece no DOMINGO. E sabado a noite
    e' o horario mais cheio de barbearia: a barra do dia mais movimentado da
    semana migraria inteira para o dia seguinte, com o total do periodo
    fechando certo e cada barra errada.

    O `override_settings(TIME_ZONE="UTC")` e' o que faz este teste valer
    alguma coisa. Sem ele o teste passa MESMO SEM o `tzinfo=FUSO`, porque o
    `TruncDate` do Django cai no fuso corrente, que hoje ja e' America/
    Sao_Paulo pelo settings — medido, nao suposto. Ou seja: sem esta linha o
    teste afirmaria o comportamento e nao a implementacao, e o dia em que
    alguem trocasse `TIME_ZONE` a serie quebraria em silencio.

    E' tambem a regra da casa: fuso e' so' `tenant/datas.py`. `FUSO` explicito
    prende o agrupamento aquele arquivo, e nao a uma configuracao do Django que
    outra pessoa pode mexer por outro motivo.
    """
    from django.test import override_settings

    from app.services.resumo import serie_por_dia
    ...
    # 21h de 05/09 no horario de Sao Paulo = 00h de 06/09 em UTC.
    inicio = local_para_utc("2026-09-05", 21 * 60)
    assert inicio.astimezone(timezone.utc).day == 6, "premissa: em UTC isto ja e dia 6"

    _agendamento(b.id, barbeiro, cliente, servico, inicio)

    with override_settings(TIME_ZONE="UTC"):
        serie = serie_por_dia(str(b.id), "2026-09-01", "2026-09-30",
                              datetime(2026, 9, 10, 12, 0, tzinfo=timezone.utc))

    assert [(x["dia"], x["cortes"]) for x in serie] == [("2026-09-05", 1)]


def test_a_serie_de_um_barbeiro_ignora_os_outros(cenario):
    """E' o ponto da tela: escolher um barbeiro e ver os cortes DELE."""
    from app.services.resumo import serie_por_dia
    from tenant.datas import local_para_utc

    b = cenario["brutus"]
    um = _barbeiro(b.id, nome="Um", ordem=1)
    outro = _barbeiro(b.id, nome="Outro", ordem=2)
    cliente = _cliente(b.id)
    servico = _servico(b.id)

    _agendamento(b.id, um, cliente, servico, local_para_utc("2026-09-01", 9 * 60))
    _agendamento(b.id, um, cliente, servico, local_para_utc("2026-09-02", 9 * 60))
    _agendamento(b.id, outro, cliente, servico, local_para_utc("2026-09-01", 10 * 60))

    agora = datetime(2026, 9, 10, 12, 0, tzinfo=timezone.utc)

    do_um = serie_por_dia(str(b.id), "2026-09-01", "2026-09-30", agora, barbeiro_id=um.id)
    assert [(x["dia"], x["cortes"]) for x in do_um] == [("2026-09-01", 1), ("2026-09-02", 1)]

    toda = serie_por_dia(str(b.id), "2026-09-01", "2026-09-30", agora)
    assert [(x["dia"], x["cortes"]) for x in toda] == [("2026-09-01", 2), ("2026-09-02", 1)]


def test_a_serie_so_traz_dia_com_corte(cenario):
    """Dia vazio nao vem, e e' a tela que decide se ele vira barra zero ou
    buraco — ela ja sabe somar dias. Trazer 366 zeros para uma janela de um
    ano seria payload por nada."""
    from app.services.resumo import serie_por_dia
    from tenant.datas import local_para_utc
    ...
    cliente = _cliente(b.id)
    servico = _servico(b.id)
    _agendamento(b.id, barbeiro, cliente, servico, local_para_utc("2026-09-03", 9 * 60))
...
def test_a_serie_nao_conta_o_que_ainda_nao_terminou(cenario):
    """Mesma regra do resto do resumo: corte que nao aconteceu nao e' corte
    feito. Sem isto a barra de hoje nasceria alta de manha e cairia."""
    from app.services.resumo import serie_por_dia
    from tenant.datas import local_para_utc
    ...
    cliente = _cliente(b.id)
    servico = _servico(b.id)
    _agendamento(b.id, barbeiro, cliente, servico, local_para_utc(DIA, 9 * 60))
    _agendamento(b.id, barbeiro, cliente, servico, local_para_utc(DIA, 18 * 60))

    serie = serie_por_dia(str(b.id), DIA, DIA, AGORA)
    assert [(x["dia"], x["cortes"]) for x in serie] == [(DIA, 1)]
