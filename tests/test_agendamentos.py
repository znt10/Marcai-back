...
def test_a_rota_do_ics_devolve_text_calendar(client, cenario):
    """O ponto INTEIRO desta rota. A tela ja montava o mesmo arquivo num
    `Blob` com `a.download` — que o Safari do iOS ignora em URL `blob:`, e o
    botao nao fazia nada no iPhone. Servido daqui, com este content-type, o
    iOS abre a folha do Calendario."""
    b = cenario["brutus"]
    barbeiro = _barbeiro(b.id)
    a = _agendamento(b.id, barbeiro, datetime(2026, 9, 10, 12, 0, tzinfo=timezone.utc))
    codigo = a.codigo

    r = client.get(f"/api/agendamentos/{codigo}/ics", headers={"host": "brutus.localhost"})

    assert r.status_code == 200
    assert r["Content-Type"] == "text/calendar; charset=utf-8"
    assert "attachment" in r["Content-Disposition"]
    assert codigo in r["Content-Disposition"]
    corpo = r.content.decode()
    assert corpo.startswith("BEGIN:VCALENDAR")
    assert f"UID:{codigo}@marcai" in corpo


def test_o_ics_de_outra_barbearia_nao_e_encontrado(client, cenario):
    """Mesma garantia do detalhe e do cancelamento ao lado, e quem a da' e' o
    RLS, nao um filtro nesta view."""
    b = cenario["brutus"]
    barbeiro = _barbeiro(b.id)
    codigo = _agendamento(b.id, barbeiro, datetime(2026, 9, 10, 12, 0, tzinfo=timezone.utc)).codigo

    r = client.get(f"/api/agendamentos/{codigo}/ics", headers={"host": "dontony.localhost"})
    assert r.status_code == 404


def test_o_ics_de_codigo_que_nao_existe_da_404(client, cenario):
    r = client.get("/api/agendamentos/naoexiste/ics", headers={"host": "brutus.localhost"})
    assert r.status_code == 404
